...
__global__ void tiling_double_buffer(float *a, float *b, float *c, int m, int n, int k) 
{
    ...
    // All threads cooperate to load tile 0 into buffer 0.
    {
        ...
        // This loading logic is already coalesced.
    ...
    for (int t = 0; t < num_tiles; t++) {
        ...
        // Start loading the next tile (coalesced)
        if (t < num_tiles - 1) {
    ...
    // Step 1: Write results from registers (sum) to shared memory (s_c)
    // This write uses the compute-mapping (uncoalesced)
    for (int j = 0; j < REG_TILE_WIDTH; j++) {
        int output_col = threadIdx.x * REG_TILE_WIDTH + j;
        s_c[threadIdx.y][output_col] = sum[j]; 
    }

    __syncthreads(); // Wait for all sums to be in shared memory

    // Step 2: Read from shared memory and write to global memory (c)
    // This write uses the 1D coalesced-mapping (same as loading)
    
    // Note: thread_idx_1d and block_size are already defined above
    for (int i = 0; i < 4; i++) { // Each thread writes 4 elements
        int tile_idx = thread_idx_1d + i * block_size;
